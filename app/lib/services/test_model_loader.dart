...
Future<void> testModelIntegrity() async {
  ...
  final files = {
    'vision_encoder_fp16.onnx': 215 * 1024 * 1024,
    'embed_tokens_fp16.onnx': 25 * 1024 * 1024,
    'decoder_model_merged_fp16.onnx': 900 * 1024 * 1024,
  ...
  for (final entry in files.entries) {
    ...
    if (await file.exists()) {
      ...
      if (size >= minSize) {
        print(
          '✅ ${entry.key} → ${(size / (1024 * 1024)).toStringAsFixed(2)} MB',
        );
      } else {
        print(
          '⚠️ ${entry.key} too small: ${(size / (1024 * 1024)).toStringAsFixed(2)} MB < ${(minSize / (1024 * 1024)).toStringAsFixed(2)} MB',
        );
        allOk = false;
      }
    } else {
